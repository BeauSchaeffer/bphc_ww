...
sites_depth_lt10=$(awk '$4 < 10 {n++} END {print n+0}' "$depth_file")

total_sites=$(awk 'END {print NR}' "$depth_file")

frac_genome_cov_10x=$(awk -v total="$total_sites" '$4 >= 10 {n++} END {if (total > 0) printf "%.4f", n/total; else print "NA"}' "$depth_file")

# spike gene metrics
spike_avg_depth=$(awk -v s="$SPIKE_START" -v e="$SPIKE_END" \
    '$2 >= s && $2 <= e {sum += $4; n++} END {if (n > 0) printf "%.2f", sum/n; else print "NA"}' "$depth_file")

spike_sites_depth_lt10=$(awk -v s="$SPIKE_START" -v e="$SPIKE_END" \
    '$2 >= s && $2 <= e && $4 < 10 {n++} END {print n+0}' "$depth_file")

spike_frac_cov_10x=$(awk -v s="$SPIKE_START" -v e="$SPIKE_END" \
    '$2 >= s && $2 <= e {total++; if ($4 >= 10) n++} END {if (total > 0) printf "%.4f", n/total; else print "NA"}' "$depth_file")

# write header + results to TSV
echo -e "sample\tavg_depth\tsites_depth_lt10\ttotal_sites\tfrac_genome_cov_10x\tspike_avg_depth\tspike_sites_depth_lt10\tspike_frac_cov_10x" > "$outfile"
echo -e "${sample}\t${avg_depth}\t${sites_depth_lt10}\t${total_sites}\t${frac_genome_cov_10x}\t${spike_avg_depth}\t${spike_sites_depth_lt10}\t${spike_frac_cov_10x}" >> "$outfile"
